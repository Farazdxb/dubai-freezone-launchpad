...
export interface CampaignConfig {
  promo?: { text: string; priceOld?: string; priceNew: string };
  hero: {
    eyebrow: string;
    title: ReactNode;
    subtitle: string;
...
function VideoShowcase({ title, subtitle }: { title: ReactNode; subtitle: string }) {
    ...
    <section className="relative overflow-hidden py-20 lg:py-32 bg-black">
      ...
      <div className="container-wide relative">
        <div className="text-center max-w-2xl mx-auto mb-12 lg:mb-16">
          <p className="text-xs uppercase tracking-[0.25em] text-white/60 font-semibold mb-4">See It In Action</p>
          <h2 className="serif-display text-4xl sm:text-5xl lg:text-6xl text-white">{title}</h2>
          <p className="mt-5 text-white/60 text-base lg:text-lg leading-relaxed">{subtitle}</p>
        </div>
        <motion.div
                ...
                >
                  ...
                  <span className="absolute inset-0 flex items-center justify-center">
                    <span className="relative flex h-20 w-20 sm:h-24 sm:w-24 lg:h-28 lg:w-28 items-center justify-center">
                      ...
                      <svg viewBox="0 0 24 24" className="relative h-7 w-7 sm:h-8 sm:w-8 lg:h-10 lg:w-10 translate-x-[2px] text-black" fill="currentColor">
...
export function CampaignTemplate({ config }: { config: CampaignConfig }) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    toast({ title: "Request received", description: "Our team will contact you within 10 minutes." });
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      {config.promo && (
        <div className="w-full bg-foreground text-white text-center text-xs sm:text-sm py-2.5 px-4">
          <span className="inline-flex items-center gap-2 flex-wrap justify-center">
            ...
            <span>{config.promo.text}</span>
            {config.promo.priceOld && (
              <span className="line-through opacity-60">{config.promo.priceOld}</span>
            )}
            <span className="font-semibold text-primary-glow">{config.promo.priceNew}</span>
          </span>
        </div>
      )}
      ...
      <section className="relative overflow-hidden pt-12 lg:pt-20 pb-20 lg:pb-28 bg-secondary">
        ...
        <div className="container-wide relative grid lg:grid-cols-2 gap-12 lg:gap-16 items-start">
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            ...
            <p className="mt-6 text-base sm:text-lg text-muted-foreground max-w-xl leading-relaxed">{hero.subtitle}</p>

            {/* Price row */}
            <div className="mt-8 flex items-center gap-4 flex-wrap">
              <span className="serif-display text-4xl sm:text-5xl font-semibold text-foreground">{hero.priceNew}</span>
              {hero.priceOld && (
                <span className="text-lg sm:text-xl text-muted-foreground line-through">{hero.priceOld}</span>
              )}
              {hero.discountLabel && (
            ...
            {(hero.timeline || hero.bestFor) && (
              <div className="mt-5 flex flex-col gap-2 text-sm text-foreground/75">
                {hero.timeline && (
                  <span className="inline-flex items-center gap-2"><Clock className="w-4 h-4 text-primary" /> <strong className="font-semibold">Timeline:</strong> {hero.timeline}</span>
                )}
                {hero.bestFor && (
                  <span className="inline-flex items-start gap-2"><Sparkles className="w-4 h-4 text-primary mt-0.5" /> <span><strong className="font-semibold">Best for:</strong> {hero.bestFor}</span></span>
                )}
              </div>
            )}
            ...
            <div className="mt-10">
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-foreground/60 mb-4">
                {hero.includedLabel || "What's included"}
              </p>
              <ul className="grid sm:grid-cols-2 gap-x-6 gap-y-3">
                {hero.includedItems.map((item) => (
            ...
            <div className="mt-8 flex items-center gap-4 bg-background border border-border rounded-2xl px-4 py-3 shadow-sm w-fit">
              <div className="flex -space-x-2">
                {[
                  ...
                  <img key={i} src={src} alt="Happy customer" className="w-9 h-9 rounded-full border-2 border-background object-cover" />
                ))}
              </div>
          ...
          >
            ...
            <div className="bg-card rounded-3xl border border-border shadow-xl p-5 sm:p-6">
              <h2 className="serif-display text-xl sm:text-2xl text-foreground leading-tight">{config.formTitle}</h2>
              ...
              <form onSubmit={handleSubmit} className="mt-5 space-y-3">
                <div>
                  ...
                  <Input id="name" required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="John Smith" className="mt-1 h-10 rounded-lg text-sm" />
                </div>
                <div>
                  <Label htmlFor="phone" className="text-[11px] font-medium text-foreground/70">Phone / WhatsApp Number</Label>
                  <Input id="phone" required type="tel" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} placeholder="+971 50 000 0000" className="mt-1 h-10 rounded-lg text-sm" />
                </div>
                <div>
                  <Label htmlFor="email" className="text-[11px] font-medium text-foreground/70">Email Address</Label>
                  <Input id="email" required type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} placeholder="you@company.com" className="mt-1 h-10 rounded-lg text-sm" />
                </div>
                <div>
                  <Label htmlFor="activity" className="text-[11px] font-medium text-foreground/70">Business Activity</Label>
                  <Input id="activity" value={form.activity} onChange={(e) => setForm({ ...form, activity: e.target.value })} placeholder="e.g. Trading, Consulting, IT Services..." className="mt-1 h-10 rounded-lg text-sm" />
                </div>
                <Button type="submit" className="w-full rounded-lg h-11 font-semibold shadow-lg shadow-primary/25 mt-1">
        ...
        <div className="container-wide py-12 lg:py-16">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-4 text-center">
            {config.stats.map((s, i) => (
              <motion.div key={s.label} initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: i * 0.05 }}>
                <div className="serif-display text-3xl sm:text-4xl lg:text-5xl text-white">{s.value}</div>
                <div className="mt-2 text-xs sm:text-sm uppercase tracking-[0.15em] text-white/60">{s.label}</div>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <section className="py-20 lg:py-28 bg-background">
        <div className="container-wide">
          <div className="text-center max-w-2xl mx-auto mb-16">
            ...
            <h2 className="serif-display text-4xl sm:text-5xl lg:text-6xl text-foreground">{config.stepsTitle}</h2>
          </div>
          <div className="grid md:grid-cols-3 gap-6">
            {config.steps.map((step, i) => (
              ...
              >
                <div aria-hidden className="serif-display absolute -top-6 -right-2 text-[140px] leading-none font-semibold text-primary/[0.06] select-none">
                  {step.n}
                </div>
      ...
      <VideoShowcase title={config.videoTitle} subtitle={config.videoSubtitle} />
      ...
      <section className="py-20 lg:py-28 bg-secondary">
        <div className="container-wide">
          <div className="text-center max-w-2xl mx-auto mb-16">
            ...
            <h2 className="serif-display text-4xl sm:text-5xl lg:text-6xl text-foreground">{config.reviewsTitle}</h2>
          </div>
          <div className="grid md:grid-cols-3 gap-6">
      ...
      <section className="py-20 bg-background">
        <div className="container-wide max-w-4xl">
          <h2 className="serif-display text-3xl sm:text-4xl text-foreground mb-8 text-center">Important Information</h2>
      ...
      {/* Sticky CTA */}
      <div className={`fixed bottom-0 left-0 right-0 z-40 transition-transform duration-300 ${formInView ? "translate-y-full lg:translate-y-0" : "translate-y-0"}`}>
        <div className="bg-foreground/95 backdrop-blur-xl border-t border-white/10">
          <div className="container-wide py-3 sm:py-4 flex items-center justify-between gap-3 flex-wrap">
            <p className="hidden sm:block text-sm sm:text-base text-white font-medium">{config.stickyCtaText}</p>
            <div className="flex items-center gap-2 sm:gap-3 w-full sm:w-auto">
              <Button onClick={() => document.getElementById("lead-form")?.scrollIntoView({ behavior: "smooth" })} className="rounded-full px-5 sm:px-6 h-11 font-semibold flex-1 sm:flex-none shadow-lg shadow-primary/30">
                {config.ctaApplyText} <ArrowRight className="w-4 h-4" />
              </Button>
              <Button asChild className="rounded-full px-5 sm:px-6 h-11 font-semibold bg-[#25D366] text-white hover:bg-[#1faa50] flex-1 sm:flex-none">
                <a href={WHATSAPP_URL} target="_blank" rel="noreferrer">
                  <MessageCircle className="w-4 h-4" />
